    ...
    <div className="flex h-full flex-col items-center justify-center gap-6 bg-[var(--bg)]">
      ...
      <div className="flex gap-3">
        {!isSessionExpired && (
          <button
            onClick={retry}
            className="flex items-center gap-2 rounded-xl bg-[var(--bg-brand)] px-6 py-2.5 text-sm font-medium text-[var(--accent)] hover:bg-[var(--bg-brand-hover)] transition-colors"
          >
            <RefreshCw size={16} />
            Reconnect
          </button>
        ...
        <button
          ...
          className={`flex items-center gap-2 rounded-xl px-6 py-2.5 text-sm font-medium transition-colors ${
            isSessionExpired
              ? 'bg-[var(--bg-brand)] text-[var(--accent)] hover:bg-[var(--bg-brand-hover)]'
